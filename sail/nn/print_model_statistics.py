...
def _format_size(x: int, sig_figs: int = 3, hide_zero: bool = False) -> str:
    ...
    if abs(x) > 100000000000000.0:
        return fmt(x / 1000000000000000.0) + "P"
    if abs(x) > 100000000000.0:
        return fmt(x / 1000000000000.0) + "T"
    if abs(x) > 100000000.0:
        return fmt(x / 1000000000.0) + "G"
    if abs(x) > 100000.0:
        return fmt(x / 1000000.0) + "M"
    if abs(x) > 100.0:
        return fmt(x / 1000.0) + "K"
    return str(x)
...
def _remove_zero_statistics(
    ...
    def keep_stat(name: str) -> None:
        ...
        if (
            ...
            or require_trivial_children
            and not trivial_children
        ):
...
def _fill_missing_statistics(
    model: paddle.nn.Layer, statistics: Dict[str, Dict[str, int]]
) -> Dict[str, Dict[str, int]]:
    """
    ...
    for mod_name, _ in model.named_sublayers():
...
def _model_stats_str(
    model: paddle.nn.Layer, statistics: Dict[str, Dict[str, str]]
) -> str:
    """
    ...
    def repr_with_statistics(module: paddle.nn.Layer, name: str) -> str:
        extra_lines = []
...
def _get_input_sizes(iterable: Iterable[Any]) -> List[Any]:
    """
...
def flop_count_str(
    ...
    model_string = ""
    if all_uncalled:
        model_string += """N/A indicates a possibly missing statistic due to how the module was called. Missing values are still included in the parent's total.
"""
...
def flop_count_table(
    ...
    return _model_stats_table(
        statistics=stats, max_depth=max_depth, stat_columns=stat_columns
    )

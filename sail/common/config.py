    ...
    def load_yaml_with_base(
        ...
        with cls._open_cfg(filename) as f:
            try:
                ...
                logger.warning(
                    "Loading config {} with yaml.unsafe_load. Your machine may be at risk if the file contains malicious content.".format(
                        filename
                    )
                )
                f.close()
        ...
        if name.startswith("COMPUTED_"):
            if name in self:
                ...
                if old_val == val:
                    return
                raise KeyError(
                    "Computed attributed '{}' already exists with a different value! old={}, new={}.".format(
                        name, old_val, val
                    )
                )
            self[name] = val

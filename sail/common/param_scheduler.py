        ...
        * milestones=[3, 6, 9], values=[1, 0.1, 0.01]  (ParamScheduler is scale-invariant)
        ...
        for milestone in self._milestones:
            if milestone >= self._num_updates:
                raise ValueError(
                    "Milestone must be smaller than total number of updates: num_updates=%d, milestone=%d"
                    % (self._num_updates, milestone)
                )
...
class StepWithFixedGammaParamScheduler(ParamScheduler):
    ...
    def __init__(
        self, base_value: float, num_decays: int, gamma: float, num_updates: int
    ) -> None:
...
class CompositeParamScheduler(ParamScheduler):
    ...
    def __call__(self, where: float) -> float:
        ...
        while (
            where + self.WHERE_EPSILON > running_total and i < len(self._schedulers) - 1
        ):
            i += 1
            running_total += self._lengths[i]

...
PADDLE_VERSION: Tuple[int, ...] = tuple(
    int(x) for x in paddle.__version__.split(".")[:2]
)
if PADDLE_VERSION >= (2, 6):
    from paddle import quantization
    from paddle.quantization import BaseQuantizer, BaseObserver
...
class Checkpointer:
    ...
    def save(self, name: str, **kwargs: Any) -> None:
        ...
        with self.path_manager.open(save_file, "wb") as f:
            paddle.save(obj=data, path=cast(IO[bytes], f))
    ...
    def load(
        ...
        for key in self.checkpointables if checkpointables is None else checkpointables:
            if key in checkpoint:
                ...
                obj.set_state_dict(state_dict=checkpoint.pop(key))
        return checkpoint
    ...
    def _load_file(self, f: str) -> Dict[str, Any]:
        ...
        with self.path_manager.open(f, "rb") as file:
            return paddle.load(path=cast(IO[bytes], file))
    ...
    def _load_model(self, checkpoint: Any) -> _IncompatibleKeys:
        ...
        for k in list(checkpoint_state_dict.keys()):
            if k in model_state_dict:
                ...
                if PADDLE_VERSION >= (1, 8) and isinstance(
                    model_param, paddle.nn.UninitializedParameter
                ):
                    continue
                ...
                if shape_model != shape_checkpoint:
                    ...
                    if has_observer_base_classes:
                        ...
                        cls_to_skip = BaseObserver, BaseQuantizer
                        target_module = _get_module_for_key(self.model, k)
                        if isinstance(target_module, cls_to_skip):
                            continue
                    incorrect_shapes.append((k, shape_checkpoint, shape_model))
                    checkpoint_state_dict.pop(k)
        incompatible = self.model.set_state_dict(
            state_dict=checkpoint_state_dict, use_structured_name=False
        )
    ...
    def _log_incompatible_keys(self, incompatible: _IncompatibleKeys) -> None:
        ...
        for k, shape_checkpoint, shape_model in incompatible.incorrect_shapes:
            self.logger.warning(
                "Skip loading parameter '{}' to the model due to incompatible shapes: {} in the checkpoint but {} in the model! You might want to double check if this is expected.".format(
    ...
    def _convert_ndarray_to_tensor(self, state_dict: Dict[str, Any]) -> None:
        ...
        for k in list(state_dict.keys()):
            v = state_dict[k]
            if not isinstance(v, np.ndarray) and not isinstance(v, paddle.Tensor):
                raise ValueError(
                    "Unsupported type found in checkpoint! {}: {}".format(k, type(v))
                )
            if not isinstance(v, paddle.Tensor):
                state_dict[k] = paddle.to_tensor(data=v)
...
def _filter_reused_missing_keys(model: paddle.nn.Layer, keys: List[str]) -> List[str]:
    """
    ...
    for module_prefix, module in _named_modules_with_dup(model):
        for name, param in list(
            module.named_parameters(include_sublayers=False)
        ) + list(module.named_buffers(include_sublayers=False)):
...
def _named_modules_with_dup(
    model: paddle.nn.Layer, prefix: str = ""
